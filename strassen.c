...
// Used when matrix size is smaller than 16 to increase efficiency for Strassen Algorithm.
...
// Form a matrix from four sub-matrices (C11 C12 C21 C22), and avoid copying last row and column if we padded zero for odd N.
void four_blocks_to_matrix( int N, int padding, double ** matrix, double ** C11, double ** C12, double ** C21, double ** C22 )
{
    ...
    for( int i = 0 ; i < N ; i++ )
        for( int j = 0 ; j < (N-padding) ; j++ )
            matrix[i][N+j] = C12[i][j];

    for( int i = 0 ; i < (N-padding) ; i++ )
        for( int j = 0 ; j < N ; j++ )
            matrix[N+i][j] = C21[i][j];

    for( int i = 0 ; i < (N-padding) ; i++ )
        for( int j = 0 ; j < (N-padding) ; j++ )
            matrix[N+i][N+j] = C22[i][j];
}
...
void Strassen(int n, double** A, double** B, double** C) {
	...
	// Use sequential mutiplication when matrix size is smaller than 16 to increase efficiency.
	// The best threshold might vary between different platforms.
	if(n<=16){
	...
	// Cannot move allocation memory to another function for clean code because it will encounter Illegal instruction error 
	...
	// Allocate two dimentional array in this fashion to avoid memory fragmentation 
	// Use calloc to initialize as 0 for padding
	...
	if( (trueA11==NULL) || (trueA12==NULL) || (trueA21==NULL) || (trueA22==NULL) || 
	(trueB11==NULL) || (trueB12==NULL) || (trueB21==NULL) || (trueB22==NULL) || 
	(trueC11==NULL) || (trueC12==NULL) || (trueC21==NULL) || (trueC22==NULL) ){
	...
	// Copy original matrix A to local four sub-matrix, and avoid copying the last row and column if we use padding
	...
	// Copy original matrix B to local four sub-matrix, and avoid copying the last row and column if we use padding
	...
	//Refer to "Memory efficient scheduling of Strassen-Winograd’s matrix multiplication algorithm"
	//Table 3: IP schedule for operation C <- A×B in place
	//http://lig-membres.imag.fr/pernet/Publications/fp05-dumas.pdf
	...
	if (raised & FE_OVERFLOW) { printf(" Overflow detected !\n");}
	if (raised & FE_UNDERFLOW) { printf(" Underflow detected !\n"); }
	// NOTE: This might not work due to compiler and optimization. BUG : https://llvm.org/bugs/show_bug.cgi?id=6050
#endif

	// Put the calculation result of four sub-matrices (C11 C12 C21 C22) into one matrix C
	// Avoid copying the last row and column if we use padding 
	four_blocks_to_matrix( N, padding, C, C11, C12, C21, C22);

	//free memory
	//Cannot move free memory to another function for clean code because it will encounter Illegal instruction error 
	free(A11[0]);
	free(A12[0]);
	free(A21[0]);
	free(A22[0]);
