...
int main(int argc, char **argv) {
  ...
  // decide whether we want to run sequential matrix multiplication for
  // verification.
  if (argc >= 3) verification = atoi(argv[2]);
  ...
  double **T = (double **)malloc(
      sizeof(double *) *
      N);  // Store the correct answer for verifying the correctness of C
  ...
  if ((trueA == NULL) || (trueB == NULL) || (trueC == NULL) ||
      (trueT == NULL)) {
    printf("malloc failed ! Out of memory !\n");
    exit(EXIT_FAILURE);
  }
  ...
  clock_t begin;
  clock_t end;
  double traditional_time_spent;  // store calculation time for traditional
                                  // sequential method
  double fastMatrixMultiplication_time_spent;  // store calculation time for
                                               // this implementation
  ...
  if (verification) {
    for (int i = 0; i < N; ++i) {
      for (int j = 0; j < N; ++j) {
        if (T[i][j] != C[i][j]) {
          printf(
              "ERROR !! Calculation result DID NOT match! Correct is %F, and "
              "we got %F\n",
              C[i][j], T[i][j]);
          exit(EXIT_FAILURE);
        }
    ...
    printf(
        "\nThis implementation by Strassen Algorithm spent %F seconds for %d X "
        "%d matrices.\n\n",
